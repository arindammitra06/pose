...
var fingercount;
var modelcolor = 'white';
var drawingguidecolor = 'rgb(61, 61, 61)';
...
class HeadShape extends ParametricShape
{
	constructor(feminine, params) {
		super(Mannequin.texHead, modelcolor, function(u, v, target) {
			...
			target.set(r * params[0] * cos(u) * cos(v), r * params[1] * sin(u) * cos(v), (r + k) * params[2] * sin(v));
		}, 32, 32);
...
class ShoeShape extends THREE.Group
{
	...
	{
		super();
			this.add(new ParametricShape(Mannequin.texLimb, modelcolor, function(u, v, target) {
				var r = Mannequin.cossers(u, v, [
					[0.6, 1.1, 0.05, 0.95, 1],
				...
				u = 360 * u;
				v = 180 * v - 90;
				target.set((3 * r - 2) * params[0] * (cos(u) * cos(v) + (feminine ? (Math.pow(sin(u + 180), 2) * cos(v) - 1) : 0)) - (feminine ? 0 : 2), params[1] * sin(u) * cos(v) + 2, params[2] * sin(v));
			}, 24, 12));
			if (feminine) {
				this.add(new ParametricShape(Mannequin.texLimb, modelcolor, function(u, v, target) {
					var r = Mannequin.cossers(u, v, [
					...
					u = 360 * u;
					v = 180 * v - 90;
					target.set(0.3 * (3 * r - 2) * params[0] * (cos(u) * cos(v)), 0.8 * params[1] * sin(u) * cos(v) + 2, 0.6 * params[2] * sin(v));
				}, 12, 12));
...
{
	constructor(feminine, params) {
		super(Mannequin.texLimb, modelcolor, function(u, v, target) {
			var r = Mannequin.cossers(u, v, [
				[0.6, 0.95, 0, 1, 4],
			...
			u = 360 * u - 90;
			v = 180 * v - 90;
			target.set(-1.5 + r * params[0] * cos(u) * Math.pow(cos(v), 0.6), r * params[1] * sin(u) * Math.pow(cos(v), 0.6), r * params[2] * sin(v));
		}, 32, 32);
...
class LimbShape extends ParametricShape
{
	constructor(feminine, params, nU = 24, nV = 32) {
		...
		super(Mannequin.texLimb, modelcolor, function(u, v, target) {
			...
			var w = new THREE.Vector3(x * cos(v) * cos(170 * u - 85) / 2, y * (1 / 2 + sin(180 * u - 90) / 2), z * sin(v) * cos(180 * u - 90) / 2);
...
class Pelvis extends Joint
{
	constructor(parentJoint) {
		...
		var dgline = new THREE.Mesh(new THREE.CylinderGeometry(0.25, 0.25, 10, 8, 1, false, 0, 6.28), new THREE.MeshPhongMaterial({
				color: drawingguidecolor,
				shininess: 1,
				precision: 'mediump'
			}));
			...
			this.add(dgline);
			var dgline2 = new THREE.Mesh(new THREE.CylinderGeometry(0.25, 0.25, 6, 8, 1, false, 0, 6.28), new THREE.MeshPhongMaterial({
				color: drawingguidecolor,
				shininess: 1,
				precision: 'mediump'
			}));
			...
			this.add(dgline2);
			var dgline3 = new THREE.Mesh(new THREE.CylinderGeometry(0.25, 0.25, 6, 8, 1, false, 0, 6.28), new THREE.MeshPhongMaterial({
				color: drawingguidecolor,
				shininess: 1,
				precision: 'mediump'
			}));
...
class Torso extends Joint
{
	constructor(parentJoint) {
		super(parentJoint, [-2, 4, 0], [5, 17, 10, parentJoint.feminine ? 10 : 80, parentJoint.feminine ? 520 : 380, parentJoint.feminine ? 0.8 : 0.9, parentJoint.feminine ? 0.25 : 0.2], TorsoShape);
			...
			dgline.rotation.set(Math.PI / 2, 0, 0);
			this.add(dgline);
			var dgline2 = new THREE.Mesh(new THREE.CylinderGeometry(0.25, 0.25, 16, 8, 1, false, 0, 6.28), new THREE.MeshPhongMaterial({
				color: drawingguidecolor,
				shininess: 1,
				precision: 'mediump'
			}));
			...
			this.add(dgline2);
			var dgline3 = new THREE.Mesh(new THREE.CylinderGeometry(0.25, 0.25, 16, 8, 1, false, 0, 6.28), new THREE.MeshPhongMaterial({
				color: drawingguidecolor,
				shininess: 1,
				precision: 'mediump'
			}));
...
class Leg extends Joint
{
	constructor(parentJoint, leftOrRight) {
		...
		super(parentJoint, [-1, -3, 3.5 * leftOrRight], [4, 15, 4, -70, 220, 1, 0.4, 2], LimbShape);
		
		this.leftOrRight = leftOrRight;
		this.imageWrapper.rotation.set(Math.PI, 0, 0);
		var dgline = new THREE.Mesh(new THREE.CylinderGeometry(0.25, 0.25, 13, 8, 1, false, 0, 6.28), new THREE.MeshPhongMaterial({
				color: drawingguidecolor,
				shininess: 1,
				precision: 'mediump'
			}));
...
class Elbow extends Joint
{
	constructor(parentJoint) {
		...
		this.minRot = new THREE.Vector3(0, 0, 0);
		this.maxRot = new THREE.Vector3(0, 0, 150);
		var dgline = new THREE.Mesh(new THREE.CylinderGeometry(0.25, 0.25, 10.5, 8, 1, false, 0, 6.28), new THREE.MeshPhongMaterial({
				color: drawingguidecolor,
				shininess: 1,
				precision: 'mediump'
			}));
...
Mannequin.sphereTemplate = new THREE.Mesh(new THREE.SphereBufferGeometry(1, 16, 8), new THREE.MeshPhongMaterial({
	color: modelcolor,
	shininess: 1,
	precision: 'mediump'
}));
